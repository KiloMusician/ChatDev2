...
function writeStaged(relativePath, data) {
...
async function main() {
  ...
  const inputBuffer = await new Promise((resolve) => {
    let buffer = "";
  ...
  const tierExists = (id) => existingTiers.tiers.some((t) => t.id === id);
  ...
  const generatorExists = (id) => existingGenerators.generators.some((g) => g.id === id);
  ...
  const upgradeExists = (id) => existingUpgrades.upgrades.some((u) => u.id === id);
  const baseUpgrades = [
    {
      id: "efficiency_1",
