...
def write_text(path: str, content: str) -> None:
    ...
    if os.path.exists(path):
        ...
        if existing and existing.strip() == content.strip():
            print(f"📝 No changes needed: {os.path.relpath(path)}")
            return
        elif existing:
            print(f"🔄 Evolving existing file: {os.path.relpath(path)}")
...
class CascadeEngine:
    ...
    def build_plan(self, scans: Dict[str, List[PlanItem]], order: List[str]) -> CascadePlan:
        ...
        # Infrastructure-First: meaningful plan naming
        plan_id = f"cascade-{len(items):03d}-items"
        heuristics = {
