...
from core.chat_env import ChatEnv
from core.statistics import get_info
from core.utils import log_visualize, log_arguments
    ...
    def chatting(
        ...
        if need_reflect:
            ...
            if "recruiting" in phase_name:
                if seminar_conclusion and ("Yes".lower() not in seminar_conclusion.lower() and "No".lower() not in seminar_conclusion.lower()):
            ...
            if hasattr(assistant_response, 'msg') and assistant_response.msg and hasattr(assistant_response.msg, 'content'):
                seminar_conclusion = assistant_response.msg.content
            else:
                seminar_conclusion = ""

        log_visualize("**[Seminar Conclusion]**:\n\n {}".format(seminar_conclusion))
        if seminar_conclusion and isinstance(seminar_conclusion, str):
...
class LanguageChoose(Phase):
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        if self.seminar_conclusion and len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['language'] = self.seminar_conclusion.split("<INFO>")[-1].lower().replace(".", "").strip()
        elif self.seminar_conclusion and len(self.seminar_conclusion) > 0:
...
class CodeReviewModification(Phase):
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        if self.seminar_conclusion and "```".lower() in self.seminar_conclusion.lower():
            chat_env.update_codes(self.seminar_conclusion)
