import { dumpCircuits } from './error-reporter';
import fs from 'fs';
...
const sampleCircuit = {
  ...
  gates: [
    { id: 'g1', type: 'hadamard', qubits: [0], operation: (s: any) => s, consciousness_effect: 3 },
    { id: 'g2', type: 'cnot', qubits: [0, 1], operation: function (s: any) { return s; }, consciousness_effect: 5 },
  ]
};
...
setTimeout(() => {
  ...
  try {
    ...
    if (Array.isArray(gates) && gates.length === 2) {
      const hasOperation = gates.some((g: any) => Object.prototype.hasOwnProperty.call(g, 'operation'));
      if (!hasOperation) {
