...
3. **Individual Terminal Output**: I can get output from specific terminals
   using `get_terminal_output` tool (logs in `data/terminal_logs/`)
...
## 🛠️ Orchestrator CLI quick use (receipt-first)

- Preferred invocation (PowerShell):
  - `Set-Location -Path "C:\\Users\\keath\\Desktop\\Legacy\\NuSyQ-Hub"`
  - `python .\scripts\orchestrator_cli.py queue --limit 5`
  - Receipts land under `NuSyQ-Hub/state/receipts/cli/`.
- VS Code task: **NuSyQ-Hub: Orchestrator CLI** (prompts for args such as
  `queue --limit 5`, `away`, `zeta`). Use this to avoid cwd mix-ups.
- If you run the CLI from another repo’s cwd, receipts may be written there;
  anchor the cwd to NuSyQ-Hub or use the VS Code task to keep receipts in one
  place.
