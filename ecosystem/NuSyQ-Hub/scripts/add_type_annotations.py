...
def main() -> int:
    """Delegate to unified_type_fixer.py with add-annotations mode."""
    print(
        "⚠️  DEPRECATED: add_type_annotations.py is deprecated.\n"
        "   Please use: python scripts/unified_type_fixer.py --mode add-annotations\n"
    )

    args = sys.argv[1:]
    unified_args = ["python", str(PROJECT_ROOT / "scripts" / "unified_type_fixer.py")]
    unified_args.append("--mode")
    unified_args.append("add-annotations")
