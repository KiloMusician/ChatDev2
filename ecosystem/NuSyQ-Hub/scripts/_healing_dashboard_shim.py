"""[DEPRECATED] Healing dashboard — Use src/observability/health_dashboard_consolidated.py
...
⚠️  MIGRATION: Replace imports from scripts.healing_dashboard with:
...
# Deprecation warning
warnings.warn(
    "scripts/healing_dashboard.py is deprecated. "
    "Use 'python -m src.observability.health_dashboard_consolidated --category healing' instead. "
...
async def main():
    """Legacy main function — redirects to consolidated healing health checks."""
    dashboard = UnifiedHealthDashboard()

    print("\n" + "=" * 60)
    print("🔧 HEALING SYSTEM HEALTH (Consolidated)")
    print("=" * 60)
    print("⚠️  Using compatibility shim — update imports!")
    print("=" * 60 + "\n")

    # Get healing-specific health checks
    checks = await dashboard.get_category_health(HealthCategory.HEALING)
...
if __name__ == "__main__":
    ...
    print("scripts/healing_dashboard.py will be removed on 2026-04-28")
    print("Use: python -m src.observability.health_dashboard_consolidated --category healing\n")

    # Run healing-specific checks
    asyncio.run(main())
