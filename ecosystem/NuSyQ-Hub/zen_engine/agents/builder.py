...
class CodexBuilder:
    ...
    def apply_proposal(
        ...
        This is the CRITICAL method that closes the recursive learning loop!
        ...
        if proposal.confidence < min_confidence:
            logger.info(
                f"⏭️  Skipping proposal {proposal.proposed_id}: "
                f"confidence {proposal.confidence:.2%} < {min_confidence:.2%}"
            )
            return False

        logger.info(
            f"✅ Applying proposal {proposal.proposed_id} (confidence: {proposal.confidence:.2%})"
        )

        # Convert proposal to ZenRule
        from zen_engine.agents.codex_loader import ZenRule

        zen_rule = ZenRule(
            id=proposal.proposed_id,
            version=1,
            triggers={
                "errors": proposal.error_patterns,
                "command_patterns": proposal.trigger_patterns,
            },
            contexts=proposal.contexts,
            lesson={
                "short": proposal.lesson_short,
                "long": proposal.lesson_long,
            },
            suggestions=proposal.suggested_fixes,
            actions={
                "auto_fix": False,  # Start conservative, enable after validation
                "fix_strategy": "proposed_from_learning",
            },
            tags=proposal.tags,
            lore={
                "story": f"Learned from {len(proposal.supporting_events or [])} occurrences",
                "glyph": proposal.proposed_glyph or "",
            },
            meta={
                "source": "codex_builder",
                "confidence": proposal.confidence,
                "learned_from_events": proposal.supporting_events or [],
                "created_at": datetime.now().isoformat(),
            },
        )

        # Save to codex if auto_save enabled
        if auto_save:
            success = self.codex.save_rule(zen_rule, update_version=False)
            if success:
                logger.info(f"💾 Rule {zen_rule.id} learned and saved to codex!")
                return True
            else:
                logger.error(f"❌ Failed to save learned rule {zen_rule.id}")
                return False
        else:
            # Just add to in-memory rules
            self.codex.rules[zen_rule.id] = zen_rule
            self.codex._build_tag_index()
            logger.info(f"🧠 Rule {zen_rule.id} learned (in-memory only)")
            return True
    ...
    def learn_from_events(
        ...
        This is the MAIN METHOD for recursive learning!

        Args:
            events: Error events to learn from
        ...
        """
        logger.info(f"🧠 Learning from {len(events)} error events...")

        # Generate proposals
        proposals = self.analyze_events(events)
        ...
        for proposal in proposals:
            if self.apply_proposal(proposal, min_confidence, auto_save):
                applied += 1
                if auto_save:
                    saved += 1
        ...
        result = {
            ...
            "rules_saved": saved,
            "learned_rules": [p.proposed_id for p in proposals if p.confidence >= min_confidence],
        }

        logger.info(
            f"✨ Learning complete: {applied}/{len(proposals)} proposals applied, "
            f"{saved} saved to codex"
        )
