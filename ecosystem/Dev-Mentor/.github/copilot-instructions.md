...
You are assisting with the **Dev-Mentor** project. This repository is a VS Code-native mentorship system and a cyberpunk terminal RPG called **Terminal Depths**.

## 🗺️ Codebase Map
- `app/game_engine/`: Core game logic and command handlers.
- `app/backend/`: FastAPI server and API endpoints.
- `mcp/`: Model Context Protocol server for LLM tool-calling.
- `docs/commands/`: 474+ man pages for game commands.
- `agents/`: Personality-driven agent logic.

## 🎮 Playing the Game
The game runs on port 5000 (Replit) or 7337 (Local).

### Execute Command via curl
```bash
curl -X POST http://localhost:5000/api/game/command \
  -H "Content-Type: application/json" \
  -d '{"session_id": "copilot-session", "command": "ls /opt"}'
```

## 🛠️ Development Guidelines
- Follow the `_cmd_<name>` pattern in `app/game_engine/commands.py` for new commands.
- Use the `VirtualFS` for filesystem operations within the game.
- XP is awarded via `gs.add_xp(amount, skill)`.
- Output should be a list of dictionaries: `{"t": "type", "s": "text"}`.
