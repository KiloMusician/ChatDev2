...
df_raw: DataFrame = pd.read_csv(input_file, on_bad_lines="warn")
...
columns = Index(["image_path", "title", "author", "summary", "affiliation"])

for cat in ["Communication", "Evolution", "Simulation", "Organization"]:
    # Use .loc to keep DataFrame type in stubs
    df: DataFrame = cast(
        DataFrame, df_raw.loc[df_raw["AwesomeListCategory"] == cat]
    )
    ...
    first_line = pd.DataFrame(
        [[first_cover_path, first_title, first_author, first_summary, first_affiliation]],
        columns=columns,
    )
    ...
    for _, line in df[1:].iterrows():
        ...
        new_line = pd.DataFrame(
            [
                [
                    "./images/{}.png".format(image_path_list[index]),
                    line["Title"],
                    line["Authors"],
                    str(line["Abstract"]).replace("\n", ""),
                    line["Affiliation"],
                ]
            ],
            columns=columns,
